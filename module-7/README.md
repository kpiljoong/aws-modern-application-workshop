...
MythicalMysfits REST API는 JSON을 통해 API를 명시하기 위해 널리 사용되는 오픈 소스 프레임워크인 **Swagger**를 사용하여 정의 됩니다. API의 Swagger 정의는 `workshop/source/module-7/api/api-swagger.json`에 위치하고 있습니다. 이 파일을 열면 REST API와 그 안에 정의된 리소스, 메서드, 설정을 확인할 수 있습니다.
...
private generateSwaggerSpec(dnsName: string, vpcLink: apigateway.VpcLink): string {
  try {
    const schemaFilePath = path.resolve(__dirname + '/../../source/module-7/api/api-swagger.json');
...
```sh
cp ~/environment/workshop/source/module-7/app/service/* ~/environment/MythicalMysfits-BackendRepository/service/
```
...
웹사이트의 새 버전은 `~/environment/workshop/source/module-7/web`에 위치하고 있습니다. 이 새 버전을 복사하여 `workshop/web` 디렉토리에 붙여넣습니다:
...
```
mkdir ./service/
cp -r ~/environment/workshop/source/module-7/app/service/recommendations.py ./service/
```
